...
<div class="container theme-showcase" role="main">
    ...
    <form role="form" style="margin-top:20px;">
        <div class="form-group">
            <label for="name">真实名称</label>
            <input type="text" class="form-control" id="name" placeholder="请输入真实名称">
        </div>
        <div class="form-group">
            <label for="pcd">身份证号码</label>
            <input type="text" class="form-control" id="pcd" placeholder="请输入身份证号码">
        </div>
        <div class="form-group">
            <label for="tel">电话号码</label>
            <input type="text" class="form-control" id="tel" placeholder="请输入电话号码">
        </div>
        <button  type="button" onclick="window.location.href='${APP_PATH}/member/index.htm'" class="btn btn-default">上一步</button>
...
<script>
    ...
    $("#nextBtn").click(function () {
        // if($.trim(name).length==0 || $.trim(pcd).length==0 || $.trim(tel).length==0)
        // {
        //     layer.msg("输入信息不完整，请重新输入",{time:1000,icon:5,shift:6})
        // }else{
            $.ajax({
                type:"POST",
                ...
                success:function (result){
                    if(result.success){
                        ...
                        layer.msg("上传信息失败！！",{time:1000,icon:5,shift:6})
                    }
                },
