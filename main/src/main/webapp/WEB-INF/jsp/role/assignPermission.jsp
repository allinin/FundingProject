  ...
  $(function () {
      ...
      var setting={
          ...
          async:{
              ...
              autoParam:["id","name=n","level=lv"]//异步加载时，需要自动提交的父结点的参数
          },
          callback:{
   ...
   $("#assignPermissionBtn").click(function () {
       ...
       var treeObj=$.fn.zTree.getZTreeObj("treeDemo");//获取许可树的所有结点
       var checkedNodes=treeObj.getCheckedNodes(true);//获取选中的结点
       $.each(checkedNodes,function (i,n) {
           jsonObj["ids["+i+"]+"]=n.id;
       });
       if(checkedNodes.length==0)
       {
           layer.msg("请选择分配的许可权限",{time:2000,icon:5,shift:6});
       }else{
           var loadingIndex=-1;
           $.ajax({
               type:"Post",
               data:jsonObj,
               url:"${APP_PATH}/role/doAssignPermissions.do",
               before:function () {
                   loadingIndex=layer.msg("正在分配权限.....",{time:1000,icon:6})
                   return true;
               },
