...
<%--                                        <c:forEach begin="1" end="${page.totalno}" step="1" var="num">--%>
<%--                                            <li--%>
...
<script type="text/javascript">
    ...
    function queryPageUser(pageno) {
        ...
        $.ajax({
            ...
            data:jsonObj,
            beforeSend:function () {
                loadingIndex=layer.load(2,{time:2*1000});
                return true;
            },
    ...
    $("#queryBtn").click(function () {
        ...
        if($.trim(queryContent)==""){//判断文本框的值是否为空，应该使用双引号而不是null
          layer.msg("查询条件不能为空，请重新输入！",{time:2*1000,icon:5,shift:6},function () {
              $("#queryContent").focus();
          });
          return ;
        }
        jsonObj.queryContent=$.trim(queryContent); //给对象增加属性<%--在原来的条件上再加上一个属性--%>
        queryPageUser(1);
    });

    function deleteUser(id,loginacct){
        layer.confirm("确认要删除["+loginacct+"]用户吗？",{icon:3,shift:'提示'},function (cindex) {
            ...
            $.ajax({
                ...
                beforeSend:function () {
                    loadingIndex=layer.load("正在删除数据！",{time:2000,icon:5,shift:6})
                    return true;
                },
                success:function (result) {
                    layer.close(loadingIndex);
